...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Run()
    {
        ...
        if (((conserveMomentum && Mathf.Abs(body.velocity.x) > Mathf.Abs(targetSpeedX) && Mathf.Sign(body.velocity.x) == Mathf.Sign(targetSpeedX) && Mathf.Abs(targetSpeedX) > 0.01f)||_moveInput.x==0.0f && (surface % 4 >= 2 || surface % 16 >= 8)) && groundTime > -nairTime)
        {
